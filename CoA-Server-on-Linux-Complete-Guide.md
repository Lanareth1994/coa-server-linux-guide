...
14. [Updating: server, bots, client](#14-updating-server-bots-client)
...
## 14. Updating: server, bots, client

New releases drop in the Discord channels regularly. Updates are **incremental** — SQL patches + replaced files — **your characters always survive**. But the order matters and the launchers hide our old enemy (PowerShell).

### Golden rules
1. **Order: server → bots → client.** The bots verify the repack release at startup and refuse to run if it changed.
2. **Always `stop-all` and back up first:**
```bash
cd ~/Games/coa-server/Server
wine Runtime/python/python.exe -B Scripts/manage.py stop-all
cd ~/Games/coa-server && tar -czf ~/coa-backup-$(date +%F).tar.gz Server/
```
3. **Read every update launcher before running it** (Rule #1). Most are PowerShell wrappers = silent no-ops under Wine.

### 1. Server (repack) update
1. Download the **"Repack Update"** zip (NOT the full repack).
2. Extract it **into** `~/Games/coa-server/Server/` (its files land next to `Core/`, `mysql/`...).
3. `cat` the update's `.bat` (usually `Apply_Update.bat`). Two cases:
   - It calls the repack's Python (`manage.py ...`) → run that command under Wine directly.
   - It calls **PowerShell** (`powershell ... some.ps1`) → do NOT run it under Wine. The `.ps1` is a text file — `cat` it. It applies `.sql` files to the databases. Do that with the native client (the ps1 tells you which DB each file targets; usually `acore_world`, sometimes `acore_characters`):
```bash
cd <the update folder you just extracted>
for f in $(find . -name '*.sql' | sort); do
  echo "== $f"
  mysql --defaults-file=~/Games/coa-server/Server/mysql/admin-client.ini         --default-character-set=utf8mb4 acore_world < "$f"
done
```
4. **If the update replaced `Scripts/manage.py`**, your `innodb_use_native_aio=0` patch (Section 6.3) may be gone — re-check and re-apply:
```bash
grep -n innodb_use_native_aio ~/Games/coa-server/Server/Scripts/manage.py || \
sed -i "s|innodb_buffer_pool_size=256M\\nskip-log-bin|innodb_buffer_pool_size=256M\\nskip-log-bin\\ninnodb_use_native_aio=0|" \
  ~/Games/coa-server/Server/Scripts/manage.py
```

### 2. Bots update
1. The repack must already be updated, and the bots patch must match the new release.
2. Back up your settings: `cp CoA-Bots/Core/configs/modules/playerbots.conf ~/` (the patch preserves it, but be safe).
3. Stop everything (above), extract the **patch zip** next to `CoA-Bots/`.
4. `cat CoA-Bots/Update-Bots.bat` → it's a PowerShell wrapper → read `Update-Bots.ps1` instead. It does three things:
   - replaces changed files (it backs up the old ones itself),
   - applies new SQL from the patch's `Database/` folders → reuse the import loop from step 1 above against those folders,
   - appends any new config defaults to `playerbots.conf` → the ps1 lists them; add them manually (or just diff against your backup).
5. `bash install-bots.sh` can also be re-run safely — it skips everything already applied.

### 3. Client update
Extract the new **client patch** over `~/Games/coa-server/client/`, overwrite everything. 30 seconds, nothing to configure. Your `Interface/AddOns/UnBot` survives; only replace it if a bots release ships a newer one.

### Post-update checklist
```bash
grep innodb_use_native_aio Scripts/manage.py   # patch still there?
# start with bots, then in game:
/who                 # bots alive
# your character untouched?
tail -20 Core/Logs/Errors.log
```
